...
removeAllWhites([[H, _]|T], R) :-
    member(H,
           [tlineNumber, execTAB, execSpace, execNL, tunknown, tcomment1, tcomment2]), !,
    removeAllWhites(T, R).
...
countToken([], _, 0).
countToken([[H, _]|T], H, N) :-
    countToken(T, H, M),
    N is M+1.
countToken([[H, _]|T], Y, N) :-
    H\=Y,
    countToken(T, Y, N).
...
onlyWhites(Cmd) :-
    string_codes(Cmd, CodesChars),
    onlyWhitesH(CodesChars).
