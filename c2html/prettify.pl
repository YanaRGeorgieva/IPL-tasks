...
removeAllWhites([], []) :- !.
removeAllWhites([[H, _]|T], R) :-
    member(H, [execTAB, execSpace, execNL, tunknown]),
    removeAllWhites(T, R), !.
removeAllWhites([[H, B]|T], [[H, B]|R]) :-
    \+ member(H, [execTAB, execSpace, execNL, tunknown]),
    removeAllWhites(T, R), !.
...
packTillTwoDots([[ttwodots, ":"]|T], Buff, NBuff, T) :-
    append(Buff, [[ttwodots, ":"]], NBuff).
...
packTillRightParen([H|T], Buff, Res, Rest, N) :-
    N=\=0,
    H=[H1|_],
    \+ member(H1, [trightParen, tleftParen]),
    append(Buff, [H], NewBuff),
...
addNewLines([H|T], Buff, [NewBuff|R]) :-
    H=[H1|_],
    member(H1, [tsemicolon, tcomment1, tcomment2, tdo]),
...
addNewLines([H1, H2, H3, H4|T], Buff, R) :-
    ...
    H3=[H31|_],
    member(H31, [tidentifier]),
    H4=[H41|_],
    member(H41, [tleftSqParen]),
    packTillsemiColon(T, [], Res, Rest),
...
addNewLines([H], Buff, [NewBuff|R]) :-
    H=[H1|_],
    member(H1, [trightBrace]),
    append(Buff, [H, [execNL, "\n"]], NewBuff), !,
    addNewLines([], [], R).
% if {0,{;,{,,}0,};},
addNewLines([H1, H2|T], Buff, R) :-
    H1=[H11|_],
    member(H11, [tleftBrace, trightBrace]),
    H2=[H21|_],
    member(H21, [tnumber, tsemicolon, tcomma]),
...
addNewLines([H1, H2|T], Buff, [NewBuff|R]) :-
    H1=[H11|_],
    member(H11, [tif, twhile, tfor, tswitch]),
...
%%Main predicate
addNewLines(Tokens, PackedLines) :-
    addNewLines(Tokens, [], PackedLines), !.
...
addSpaces([H|T], Buff, Res) :-
    H=[H1|_],
    member(H1, [execNL, trightParen]),
    (   append(NewBuf, [[execSpace, " "]], Buff),
        append(NewBuf, [H], NewBuff)
    ;   \+ append(_, [[execSpace, " "]], Buff),
        append(Buff, [H], NewBuff)
    ), !,
...
addSpaces([H|T], Buff, Res) :-
    H=[H1|_],
    member(H1, [tcomma, tsemicolon]),
    append(NewBuf, [[execSpace, " "]], Buff),
    append(NewBuf, [H, [execSpace, " "]], NewBuff), !,
    addSpaces(T, NewBuff, Res).
% No space after (, [, {, function name, tsizeof
addSpaces([H1|T], Buff, Res) :-
    H1=[H11|_],
    member(H11, [tleftParen, tfunction, tleftBrace, tleftSqParen, tsizeof]),
    append(Buff, [H1], NewBuff), !,
    addSpaces(T, NewBuff, Res).
% i->mau();--
addSpaces([H1|T], Buff, Res) :-
    H1=[H11|_],
    member(H11, [tptr, execNL]),
...
addSpaces([H1, H2|T], Buff, Res) :-
    ...
    append(_, [H3, H4], Buff),
    H3=[H31|_],
    \+ member(H31, [tnumber, tidentifier, trightParen, trightSqParen]),
    H4=[H41|_],
    member(H41, [execSpace, tleftParen, tleftBrace, tleftSqParen]),
    append(Buff, [H1, H2], NewBuff), !,
    addSpaces(T, NewBuff, Res).
%  new line (*i, (&i
addSpaces([H1, H2|T], Buff, Res) :-
    ...
    length(Buff, 1),
    Buff=[[H31|_]],
    member(H31, [tleftParen, tleftBrace, tleftSqParen]),
    append(Buff, [H1, H2], NewBuff), !,
    addSpaces(T, NewBuff, Res).
% none before *i, &i
...
addSpaces([H1|T], Buff, Res) :-
    ...
    append(NewBuf, [[H11, Lex], [execSpace, " "]], NewBuff), !,
    addSpaces(Rest, NewBuff, Res).
% })]]
addSpaces([H1, H2|T], Buff, Res) :-
    H1=[H11|_],
    member(H11, [trightParen, trightBrace, trightSqParen]),
    H2=[H21|_],
    member(H21, [trightParen, trightBrace, trightSqParen]),
    append(Buff, [H1], NewBuff), !,
    addSpaces([H2|T], NewBuff, Res).
% 0], 0}, a], a}, 1), a), a][
addSpaces([H1, H2, H3|T], Buff, Res) :-
    H1=[H11|_],
    member(H11, [tnumber, tidentifier]),
    H2=[H21|_],
    member(H21, [trightParen, trightBrace, trightSqParen]),
    H3=[H31|_],
    ...
    ;   \+ member(H31, [trightParen, trightBrace, trightSqParen]),
        append(Buff, [H1, H2, [execSpace, " "]], NewBuff)
    ), !,
...
addSpaces([H1, H2|T], Buff, Res) :-
    ...
    member(H11, [tidentifier]),
    H2=[H21|_],
    member(H21, [tleftSqParen]),
    append(Buff, [H1], NewBuff), !,
    addSpaces([H2|T], NewBuff, Res).
%  Else
addSpaces([H|T], Buff, Res) :-
    append(Buff, [H, [execSpace, " "]], NewBuff), !,
    addSpaces(T, NewBuff, Res).
