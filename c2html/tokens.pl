...
is_quotation([H, _]) :-
    member(H, [tquot, tdoubleQuot, tcomma, tdot, tquestion]).
...
is_packedFunction([H|_]) :-
    append(_, [[tfunction|_]|_], H).

is_packedStructure([H|_]) :-
    append(_, [[tstruct|_]|_], H).

% experimental is_functionWithCC([H|_]):- H = [[tCC]|_].
is_identifier([H, _]) :-
    member(H, [tidentifier, tnl, ttab, tspace]).

is_blank([H, _]) :-
    member(H, [execTAB, execSpace, execNL]).

is_comment([H, _]) :-
    member(H, [tcomment1, tcomment2]).
